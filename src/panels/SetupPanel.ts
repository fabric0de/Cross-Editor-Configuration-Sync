import { Disposable, Webview, WebviewPanel, window, Uri, ViewColumn, SecretStorage, authentication, workspace, commands } from "vscode";
...
export class SetupPanel {
  ...
  public static render(extensionUri: Uri, secrets: SecretStorage) {
    if (SetupPanel.currentPanel) {
      ...
      const panel = window.createWebviewPanel(
        ...
        {
          enableScripts: true,
          localResourceRoots: [Uri.joinPath(extensionUri, "out"), Uri.joinPath(extensionUri, "media")],
        }
      );
        ...
        switch (message.type) {
          ...
          case "changeLocalPath": {
            ...
            if (uri && uri[0]) {
              ...
              await workspace.getConfiguration('cecs').update('localBackupPath', newPath, true);
              window.showInformationMessage(`✅ Storage path changed. Please reconnect local storage.`);
          ...
          case "connectLocal": {
            ...
            const finalPath = customPath ? `${customPath}/config.json` : `${home}/.cecs/config.json`;
  ...
  private async _addProvider(type: 'gist'|'local', name: string, config?: any) {
      try {
          ...
          let providers: any[] = existingStr ? JSON.parse(existingStr) : [];
      ...
      try {
          ...
          if (providersStr) {
              ...
              try {
                  ...
                  if (oldType) {
                      const home = process.env.HOME || process.env.USERPROFILE || '';
                      const legacyConfig = oldType === 'local' ? { path: `${home}/.cecs/config.json` } : undefined;
  ...
  private async _startGitHubAuth() {
    try {
      ...
      if (session) {
        ...
        window.showInformationMessage(`✅ GitHub login successful! (${session.account.label})`);
        this._sendState();
      }
    } catch (error: any) {
