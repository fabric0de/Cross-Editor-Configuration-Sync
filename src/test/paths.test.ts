...
describe('Paths Core Logic', () => {
    ...
    describe('determineUserDataDir (Mac)', () => {
        ...
        it('should return correct path for Antigravity', () => {
            const result = determineUserDataDir(EditorType.Antigravity, 'Antigravity', platform, HOME);
            assert.strictEqual(result, path.join(HOME, 'Library/Application Support/Antigravity/User'));
        });

        it('should return correct path for Windsurf', () => {
            const result = determineUserDataDir(EditorType.Windsurf, 'Windsurf', platform, HOME);
            assert.strictEqual(result, path.join(HOME, 'Library/Application Support/Windsurf/User'));
        });

        it('should fallback to name-based path for Unknown editor', () => {
            const result = determineUserDataDir(EditorType.Unknown, 'My Cool Editor', platform, HOME);
            assert.strictEqual(result, path.join(HOME, 'Library/Application Support/MyCoolEditor/User'));
        });
